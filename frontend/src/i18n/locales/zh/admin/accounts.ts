export default {
    accounts: {
      ...
      cnProviders: {
        ...
        zhipuTeam: {
          title: '团队版组织 / 项目 ID',
          organization: '组织 ID（团队版，可选）',
          organizationPlaceholder: '团队版 Coding Plan 的组织 ID（org-...）',
          project: '项目 ID（团队版，与组织 ID 配对）',
          projectPlaceholder: '团队版 Coding Plan 的项目 ID（proj_...）',
          organizationInvalid: '组织 ID 格式无效或超过 128 个字符（应为 org-...）',
          projectRequired: '填写组织 ID 时必须同时填写项目 ID',
          projectInvalid: '项目 ID 格式无效或超过 128 个字符（应为 proj_...）',
          hint: '仅团队版 GLM Coding Plan 需要填写；组织 ID 与项目 ID 必须成对，且会固定走国内团队端点。个人版两个字段都留空即可。获取方式点击问号查看教程。',
          help: {
            title: '如何获取组织 / 项目 ID',
            ...
            step4: '请求 URL 中 org- 开头的一段是组织 ID、proj_ 开头的一段是项目 ID（也可在 Request Headers 中查看 bigmodel-organization / bigmodel-project），分别填入上方输入框。',
      ...
      openaiQuotaReset: {
        ...
        spendControl: '工作区额度',
        spendControlHint: 'Team/Business/Enterprise/Edu/K12 工作区 spend control 额度',
        spendControlReached: '已达到上限',
      ...
      healthProbe: {
        ...
        partial: '测活部分完成：已处理 {completed} 个，{failed} 个所在批次请求失败（{batches} 批）；已完成结果已保留。',
      ...
      inventory: {
        ...
        partial: '盘点部分完成：健康 {healthy} 个，失败 {failed} 个，跳过 {skipped} 个，获取额度 {quota} 个；另有 {requestFailed} 个账户所在的 {batches} 批请求失败。',
        requestFailedAccounts: '批次请求失败',
        partialHint: '有 {accounts} 个账户所在的 {batches} 批请求没有返回结果；已完成批次的结果已保留，可关闭后重新盘点所选账户。',
      ...
      bulkEdit: {
        ...
        filteredSelectionInfo: '将更新当前筛选条件命中的 {count} 个账号，不限于当前勾选项。',
        filteredScopeWarning: '这是按筛选条件更新，将对当前筛选命中的全部 {count} 个账号生效。',
        filteredScopeConfirm: '我确认将对以上筛选范围内的全部账号应用本次修改。',
        filteredScopeConfirmRequired: '请先确认按筛选条件更新的全部目标账号。',
      ...
      openai: {
        ...
        capabilityPromptCacheRetention: '旧版提示缓存保留',
        ...
        customUserAgentDesc:
          '为 OpenAI OAuth 账号指定可选的 Codex 出站指纹；系统会在出站时统一版本和客户端身份。留空则使用全局设置。',
