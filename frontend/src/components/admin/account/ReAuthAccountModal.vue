...
/** Re-authorize an existing OpenAI/Antigravity account from one refresh token. */
const handleValidateRefreshToken = async (refreshTokenInput: string) => {
  if (!props.account) return

  // Keep Grok's SSO/refresh-token flow isolated; its response shape and
  // credential persistence differ from OpenAI-compatible accounts.
  if (isGrok.value) {
