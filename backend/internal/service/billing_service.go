...
type ModelPricing struct {
	...
	LongContextPricingExplicit         bool     // 目录/override 显式提供长上下文字段（显式 0 也有效）
...
func (s *BillingService) initFallbackPricing() {
	...
	// Fable 5.1 reduces cache reads to $0.25 per MTok.
	...
	if s.pricingService != nil {
		...
		if litellmPricing != nil {
			...
			return s.applyModelSpecificPricingPolicy(model, &ModelPricing{
				...
				LongContextPricingExplicit:         litellmPricing.LongContextPricingExplicit,
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool) *ModelPricing {
	...
	needsLongContextPolicy := usesLegacyLongContextPricing && !pricing.LongContextPricingExplicit &&
	...
	if !needsLongContextPolicy && !needsCacheCreationPolicy && fastRatio <= 0 && !needsMaxReasoningEffortMultiplier {
		return pricing
	}
	cloned := *pricing
