...
func TestBackfillOpenAIImagesB64JSON_DownloadRequestShape(t *testing.T) {
	...
	account.Proxy = &Proxy{ID: 3, Protocol: "http", Host: "127.0.0.1", Port: 7890}
...
func TestBackfillOpenAIImagesB64JSON_ConfiguredProxyFailsClosed(t *testing.T) {
	for _, tc := range []struct {
		name  string
		proxy *Proxy
	}{
		{name: "missing relation"},
		{name: "mismatched relation", proxy: &Proxy{ID: 4, Protocol: "http", Host: "127.0.0.1", Port: 7890}},
		{name: "invalid protocol", proxy: &Proxy{ID: 3, Protocol: "file", Host: "127.0.0.1", Port: 7890}},
		{name: "missing host", proxy: &Proxy{ID: 3, Protocol: "http", Host: "", Port: 7890}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			account := b64BackfillAccount(true)
			proxyID := int64(3)
			account.ProxyID, account.Proxy = &proxyID, tc.proxy
			body := []byte(`{"data":[{"url":"https://cdn.example.com/a.png"}]}`)
			got := svc.backfillOpenAIImagesB64JSON(context.Background(), account, nil, body)
			require.Equal(t, string(body), string(got), "failed backfill preserves the upstream response")
			require.Empty(t, upstream.requests, "a configured unusable proxy must never fall back to direct access")
		})
	}
}
