...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if openAIFastPolicyTierNeedsEvaluation(body, rawTier) {
	...
	wsRetryLoop:
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			if errors.As(wsErr, &failoverErr) {
				code, typ, msg := parseOpenAIWSErrorEventFields(failoverErr.ResponseBody)
				if reason, _ := classifyOpenAIWSErrorEventFromRaw(code, typ, msg); reason == openAIWSFallbackReasonInvalidEncryptedContent {
					// A guarded socket must fail over, not recover on the same account.
					// Still remember rejected ciphertext under the entry key used by
					// the next request, not the later WS payload's rewritten cache key.
					if lineageSessionHash == "" {
						lineageSessionHash = s.GenerateSessionHash(c, lineageEntryBody)
					}
					s.markOpenAIWSInvalidEncryptedContentLineage(lineageGroupID, lineageSessionHash, collectOpenAIEncryptedContentDigestsRaw(lineageEntryBody))
				}
			}
...
func shouldForwardOpenAIResponsesViaRawChatCompletions(account *Account) bool {
	...
	if account.IsCNProvider() {
		// CN 的显式协议配置优先于异步探针 Extra；adaptive 对提供原生
		// Responses 的 DeepSeek/Kimi 走 Responses，GLM 回退 Chat Completions。
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	if account.UsesOpenAICodexProtocol() {
		...
		if promptCacheKey != "" && !preserveSession {
			...
			// Agent Identity uses the same UUID-shaped session carrier as the
			// official ChatGPT client; keep the derived value stable across the
			// one-time task re-registration retry.
	...
	if account.UsesOpenAICodexProtocol() && compatMessagesBridge {
		req.Header.Del("conversation_id")
	}
