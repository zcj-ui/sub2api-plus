...
// used (OpenAI service_tier, Anthropic usage.speed). The observed tier remains
// separate from the final outbound tier until usage recording resolves the
// account-specific billing contract.
...
// service tier. The observed response tier is kept separately on
// OpenAIForwardResult and reconciled at usage time, where the account protocol
// is available. This matters for private ChatGPT Codex responses that commonly
// echo "default" even when the OAuth request was served on the Fast tier.
