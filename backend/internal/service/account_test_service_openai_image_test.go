...
func TestAccountTestService_OpenAIImageOAuthHandlesOutputItemDoneFallback(t *testing.T) {
	...
	err := svc.testOpenAIImageOAuth(c, context.Background(), openAITestAccountWithProxy(account), "gpt-image-2", "draw a cat")
...
func TestAccountTestService_OpenAIImageAPIKeyUsesConfiguredV1BaseURL(t *testing.T) {
	...
	err := svc.testOpenAIImageAPIKey(c, context.Background(), openAITestAccountWithProxy(account), "gpt-image-2", "draw a cat")
	require.NoError(t, err)
	require.NotNil(t, upstream.lastReq)
	require.Equal(t, HTTPUpstreamProfileOpenAI, HTTPUpstreamProfileFromContext(upstream.lastReq.Context()))
