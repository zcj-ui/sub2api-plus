...
// may lower billing. The private ChatGPT Codex endpoint commonly reports
// default even for effective Fast turns, so OAuth-like credentials retain the
// final outbound tier while still recording the observed value.
...
// the selected credential's upstream response tier is authoritative, so cost
// calculation and the usage log share one billable tier.
