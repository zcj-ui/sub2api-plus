...
func TestBuildUpstreamBillingRateSnapshotItemsPreservesAPIKeyRows(t *testing.T) {
	...
	accounts := []Account{
		...
		makeAccount(3, PlatformGemini, AccountTypeAPIKey, map[string]any{"status": "failed"}),
		makeAccount(4, PlatformOpenAI, AccountTypeOAuth, map[string]any{"status": "ok"}),
	}
