...
func TestBulkUpdateEnsuresCodexFingerprintSeedWithPerRowSQL(t *testing.T) {
	...
	require.Contains(t, query, "IN ('device', 'session', 'full')")
	require.Contains(t, query, "to_jsonb(LOWER(BTRIM(")
	require.Contains(t, query, codexFingerprintSeedCanonicalPattern)
	require.Contains(t, query, service.CodexFingerprintRecoveryRequiredExtraKey)
