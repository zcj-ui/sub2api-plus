...
// scopeFilter applies the authenticated user's effective API-key group scope
// to every non-admin monitor query. An empty allowed list is intentional: it
// must produce an empty result rather than silently widening to all groups.
