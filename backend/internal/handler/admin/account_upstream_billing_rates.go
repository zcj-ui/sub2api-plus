...
// upstreamBillingRatesResponse is the compact, read-only snapshot used by
// the account table's background rate refresh. It deliberately contains no
// credentials or runtime counters.
...
// GetUpstreamBillingRates returns persisted upstream billing snapshots for
// the current account-list page. It never probes an upstream.
//
...
func (h *AccountHandler) GetUpstreamBillingRates(c *gin.Context) {
	...
	if etag := buildUpstreamBillingRatesETag(payload); etag != "" {
		c.Header("ETag", etag)
