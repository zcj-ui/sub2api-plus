...
func TestAccountHandlerListLitePreservesPlusMetadata(t *testing.T) {
	router, adminSvc := setupAccountListRouter()
	adminSvc.accounts = []service.Account{{
		ID: 11, Name: "plus-account", Platform: service.PlatformOpenAI, Type: service.AccountTypeOAuth,
		Status: service.StatusActive, Schedulable: true,
		Credentials: map[string]any{"access_token": "secret-test-token"},
		Extra:       map[string]any{service.AccountHealthProbeExtraKey: map[string]any{"status": service.AccountHealthProbeStatusFailed}},
	}}
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/v1/admin/accounts?page=1&page_size=20&lite=1", nil)
	router.ServeHTTP(rec, req)
