...
开发版 Actions Artifact 保留 14 天，不覆盖稳定版。正式工作流只接受三段式标签，例如 `v0.2.8`。
...
开发工作流从 `backend/cmd/server/VERSION` 推导下一个补丁版本，例如 `0.2.9-dev.42.1a2b3c4d`，构建 Linux、Windows、macOS 的 amd64/arm64 归档（Windows arm64 除外），并发布 `ghcr.io/zcj-ui/sub2api-plus:dev`。
...
git commit -m "chore: prepare v0.2.8"
git tag -a v0.2.8 -m "Sub2API Plus v0.2.8"
